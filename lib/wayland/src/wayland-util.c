...
WL_EXPORT void* wl_array_add(struct wl_array *array, size_t size) {
	...
	if (array->alloc > 0) { alloc = array->alloc; }
	else { alloc = 16; }

	while (alloc < array->size + size) { alloc *= 2; }

	if (array->alloc < alloc) {
		if (array->alloc > 0) {data = realloc(array->data, alloc);}
		else { data = malloc(alloc); }
...
static inline bool map_entry_is_free(union map_entry entry)
{
	return entry.next & 0x1;
}

static inline void* map_entry_get_data(union map_entry entry) { return (void*)(entry.next & ~(uintptr_t)0x3); }

static inline uint32_t map_entry_get_flags(union map_entry entry) { return (entry.next >> 1) & 0x1; }

void wl_map_init(struct wl_map *map, uint32_t side) { memset(map, 0, sizeof *map); map->side = side; }
...
static enum wl_iterator_result for_each_helper(struct wl_array *entries, wl_iterator_func_t func, void *data) {
