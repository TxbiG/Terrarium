...
// Terrarium open flags. Keep these ABI values stable for user/kernel calls.
#define TERRA_O_ACCMODE 00000003
#define TERRA_O_RDONLY 00000000
#define TERRA_O_WRONLY 00000001
#define TERRA_O_RDWR 00000002
#define TERRA_O_CREAT 00000100
#define TERRA_O_EXCL 00000200
#define TERRA_O_NOCTTY 00000400
#define TERRA_O_TRUNC 00001000
#define TERRA_O_APPEND 00002000
#define TERRA_O_NONBLOCK 00004000
#define TERRA_O_DIRECTORY 00200000
#define TERRA_O_NOFOLLOW 00400000
#define TERRA_O_CLOEXEC 02000000
...
typedef size_t (*SpecialIoctlHandler)(OpenFile *fd, uint64_t request,
                                      void *arg);
typedef size_t (*SpecialStatHandler)(OpenFile *fd, terra_stat_t *stat);
...
typedef size_t (*SpecialGetdents64)(OpenFile *fd, terra_dirent_t *dirp,
                                    unsigned int count);
...
typedef size_t (*SpecialBind)(OpenFile *fd, terra_socket_addr_t *addr, size_t len);
typedef size_t (*SpecialListen)(OpenFile *fd, int backlog);
typedef size_t (*SpecialAccept)(OpenFile *fd, terra_socket_addr_t *addr,
                                uint32_t *len);
typedef size_t (*SpecialConnect)(OpenFile *fd, terra_socket_addr_t *addr,
                                 uint32_t len);
typedef size_t (*SpecialRecvfrom)(OpenFile *fd, uint8_t *out, size_t limit,
                                  int flags, terra_socket_addr_t *addr,
                                  uint32_t *len);
typedef size_t (*SpecialGetsockopts)(OpenFile *fd, int level, int optname,
                                     void *optval, uint32_t *socklen);
typedef size_t (*SpecialGetsockname)(OpenFile *fd, terra_socket_addr_t *addr,
                                     uint32_t *addrlen);
typedef size_t (*SpecialGetpeername)(OpenFile *fd, terra_socket_addr_t *addr,
                                     uint32_t *len);
typedef size_t (*SpecialRecvMsg)(OpenFile *fd, terra_msg_header_t *msg,
                                 int flags);
typedef size_t (*SpecialSendto)(OpenFile *fd, uint8_t *in, size_t limit,
                                int flags, terra_socket_addr_t *addr, uint32_t len);
...
typedef bool (*MntStat)(MountPoint *mnt, char *filename, terra_stat_t *target,
                        char **symlinkResolve);
typedef bool (*MntLstat)(MountPoint *mnt, char *filename, terra_stat_t *target,
                         char **symlinkResolve);
