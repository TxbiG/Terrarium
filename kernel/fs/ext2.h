...
bool   ext2Stat(MountPoint *mnt, char *filename, terra_stat_t *target,
                char **symlinkResolve);
bool   ext2Lstat(MountPoint *mnt, char *filename, terra_stat_t *target,
                 char **symlinkResolve);
size_t ext2StatFd(OpenFile *fd, terra_stat_t *target);
...
bool   ext2DirAllocate(Ext2 *ext2, uint32_t inodeNum, Ext2Inode *parentDirInode,
                       ...
                       uint32_t inode);
size_t ext2Getdents64(OpenFile *file, terra_dirent_t *start,
