...
static int bytes_equal(const uint8 *buf, uint32 offset, const char *text) {
    uint32 i = 0;
    while (text[i]) {
...
static int read_sector(uint32 block_device, uint64 lba, uint8 *sector) {
...
static int mount_exfat(uint32 block_device, void *context, size_t context_size) {
...
static int mount_ext4(uint32 block_device, void *context, size_t context_size) {
...
static int mount_ntfs3(uint32 block_device, void *context, size_t context_size) {
...
static int mount_xfs(uint32 block_device, void *context, size_t context_size) {
...
const terra_diskfs_driver_t *terra_diskfs_driver_by_name(const char *name) {
    uint32 i;
    for (i = 0; i < (uint32)(sizeof(diskfs_drivers) / sizeof(diskfs_drivers[0])); ++i) {
