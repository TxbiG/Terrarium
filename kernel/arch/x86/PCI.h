...
struct PCIDevice {
  uint16_t deviceID = 0xffff;
	...
	uint16_t* capabilities; // List of capability IDs
...
inline void SetData(PCIMSICapability compat, uint32_t d){
		if(compat.msiControl & PCI_CAP_MSI_CONTROL_64){
			compat.data64 = d;
		} else {
			compat.data = d;
		}
...
inline uint32_t GetData(PCIMSICapability compat){
	if(compat.msiControl & PCI_CAP_MSI_CONTROL_64){
		return compat.data64;
	} else {
		return compat.data;
	}
}
inline void SetAddress(PCIMSICapability compat, int cpu){
	compat.addressLow = PCI_CAP_MSI_ADDRESS_BASE | (static_cast<uint32_t>(cpu) << 12);
	if(compat.msiControl & PCI_CAP_MSI_CONTROL_64){
		compat.addressHigh = 0;
	}
}
