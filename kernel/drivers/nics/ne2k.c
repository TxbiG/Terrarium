...
static int ne2k_ready;

static int ne2k_driver_init(const terra_boot_info_t *boot_info) {
    (void)boot_info;
    return ne2k_ready ? 0 : -1;
}

static int ne2k_probe(uint16_t vendor_id,
                      uint16_t device_id,
                      uintptr_t mmio_base,
                      uint16_t io_base,
                      void *context,
...
void ne2k_init(const terra_boot_info_t *boot_info) {
    (void)boot_info;
    ne2k_ready = 1;
    terra_driver_t driver = {"ne2k", TERRA_DRIVER_NETWORK, ne2k_driver_init, ne2k_shutdown, 1};
    drivers_register(&driver);
    terra_nic_driver_probe_t probe = {"ne2k", 0x10ecu, 0x8029u, ne2k_probe, 0};
